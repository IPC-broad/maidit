...
export default function PartnerDashboard() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      const { data: payoutsData } = await supabase
        .from('payouts')
        .select(`*, offer:offers(id, salary, kasambahay_profile:profiles!offers_kasambahay_id_fkey(full_name), homeowner_profile:profiles!offers_homeowner_id_fkey(full_name, city))`)
  ...
  const handleAddWorker = async () => {
    ...
    const skillsArray = workerForm.skills.split(',').map(s => s.trim()).filter(Boolean)
  ...
  const s: any = {
    ...
    tab: (active: boolean) => ({
      padding: '8px 16px', borderRadius: '20px', border: active ? 'none' : '1.5px solid #e5e7eb',
      cursor: 'pointer', fontFamily: 'sans-serif', fontSize: '.78rem', fontWeight: 700,
      background: active ? '#0d1117' : '#fff', color: active ? '#fff' : '#6b7280'
    }),
    statCard: { background: '#fff', borderRadius: '12px', padding: '14px', border: '1.5px solid #e5e7eb', flex: 1 },
    btn: { width: '100%', padding: '12px', borderRadius: '12px', border: 'none', background: '#1a6b3c', color: '#fff', fontFamily: 'sans-serif', fontSize: '.88rem', fontWeight: 700, cursor: 'pointer', marginBottom: '8px' },
    input: { width: '100%', padding: '10px 13px', border: '1.5px solid #e5e7eb', borderRadius: '10px', fontFamily: 'sans-serif', fontSize: '.84rem', outline: 'none', marginBottom: '10px', background: '#fff', color: '#111827', boxSizing: 'border-box' as const },
    lbl: { display: 'block', fontSize: '.62rem', fontWeight: 700, textTransform: 'uppercase' as const, letterSpacing: '.5px', color: '#6b7280', marginBottom: '4px' },
  }

  const statusColor: Record<string, string> = { paid: '#166534', pending: '#92400e', scheduled: '#1e40af' }
  const statusBg: Record<string, string> = { paid: '#f0fdf4', pending: '#fffbeb', scheduled: '#eff6ff' }
      ...
      <div style={s.body}>
            ...
            <div style={{ display: 'flex', gap: '10px', marginBottom: '12px' }}>
              {[
                ...
                <div key={i} style={s.statCard}>
                  <div style={{ fontSize: '.6rem', fontWeight: 700, textTransform: 'uppercase' as const, letterSpacing: '.5px', color: '#6b7280', marginBottom: '4px' }}>{stat.label}</div>
                  <div style={{ fontFamily: 'serif', fontSize: '1.3rem', fontWeight: 900, color: stat.color }}>{stat.value}</div>
                </div>
              ))}
            </div>

            <div style={s.card}>
              <div style={{ fontSize: '.65rem', fontWeight: 700, textTransform: 'uppercase' as const, letterSpacing: '.5px', color: '#6b7280', marginBottom: '10px' }}>Your stats</div>
              {[
                ...
                <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: i < 2 ? '1px solid #f3f4f6' : 'none' }}>
                  ...
                  <span style={{ fontSize: '.82rem', fontWeight: 700, color: '#111827' }}>{row.value}</span>
                </div>
              ))}
            </div>

            <div style={{ background: '#fffbeb', border: '1px solid #fde68a', borderRadius: '11px', padding: '13px 14px' }}>
              <div style={{ fontSize: '.72rem', fontWeight: 700, color: '#92400e', marginBottom: '6px' }}>💸 Payout per hire</div>
        ...
        {tab === 'workers' && (
          <>
            ...
            {saveMsg && (
              <div style={{ background: saveMsg.includes('added') ? '#f0fdf4' : '#fef2f2', border: `1px solid ${saveMsg.includes('added') ? '#bbf7d0' : '#fecaca'}`, borderRadius: '9px', padding: '10px 13px', fontSize: '.78rem', color: saveMsg.includes('added') ? '#166534' : '#dc2626', marginBottom: '12px' }}>
                {saveMsg}
              </div>
            )}
            ...
            {showAddWorker && (
              <div style={s.card}>
                <div style={{ fontSize: '.65rem', fontWeight: 700, textTransform: 'uppercase' as const, letterSpacing: '.5px', color: '#6b7280', marginBottom: '12px' }}>Worker details</div>
                <label style={s.lbl}>Full name *</label>
                <input style={s.input} placeholder="e.g. Maria Santos" value={workerForm.full_name} onChange={e => setWorkerForm(f => ({ ...f, full_name: e.target.value }))} />
                <label style={s.lbl}>Mobile *</label>
                <input style={s.input} placeholder="09XXXXXXXXX" value={workerForm.mobile} onChange={e => setWorkerForm(f => ({ ...f, mobile: e.target.value }))} />
                <label style={s.lbl}>Province *</label>
                <input style={s.input} placeholder="e.g. Samar, Leyte, Cebu" value={workerForm.province} onChange={e => setWorkerForm(f => ({ ...f, province: e.target.value }))} />
                <label style={s.lbl}>Skills (comma-separated)</label>
                <input style={s.input} placeholder="e.g. cooking, laundry, childcare" value={workerForm.skills} onChange={e => setWorkerForm(f => ({ ...f, skills: e.target.value }))} />
                <button style={{ ...s.btn, opacity: saving ? .6 : 1 }} onClick={handleAddWorker} disabled={saving}>
            ...
            {workers.length === 0 ? (
              ...
              <div key={w.id} style={s.card}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  <div>
                    <div style={{ fontSize: '.88rem', fontWeight: 700, color: '#111827', marginBottom: '2px' }}>{w.profiles?.full_name}</div>
                    <div style={{ fontSize: '.72rem', color: '#6b7280' }}>{w.province} · {w.profiles