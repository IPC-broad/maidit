...
export async function GET(req: NextRequest) {
  ...
  const [kbRes, partnerRes] = await Promise.all([
    sa.from('kasambahay').select(`
      id, profile_id, province, setup, asking_salary, status,
      has_govt_id, has_nbi, is_verified, referred_by, created_at,
      skills, availability, age, civil_status,
      profile:profiles!profile_id(id, full_name, mobile, email, city, created_at, selfie_url, verified)
    `).order('created_at', { ascending: false }),
    sa.from('partners').select('id, referral_code'),
  ])

  const kasambahay = kbRes.data ?? []
